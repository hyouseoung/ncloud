...
resource "ncloud_auto_scaling_group" "asg" {
    ...
    target_group_list            = [var.depends-on-lb-tg-list]
    zone_no_list                 = ["2"]
    wait_for_capacity_timeout    = 0
    health_check_grace_period    = 300
    depends_on = [
        /* Auto Scaling 모듈에서 ASG 리소스가 서버 생성은 완료한 후 target_group_list에 추가 */
        var.depends-on-lb-tg-list
    ]
}
...
resource "ncloud_auto_scaling_schedule" "schedule" {
    ...
    start_time              = "2022-01-21T18:00:00+0900"
